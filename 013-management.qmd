...
That said, if your platform *does* allow you to control its output, you can try to use the principles of good data design outlined above. For example, give your variables (e.g., questions in Qualtrics)\indexC{Qualtrics} sensible names! 

### Document the format of your data

Even the best-organized tabular data are not always easy to understand by other researchers, or even yourself, especially after some time has passed. As such, you should make a **codebook**\indexC{codebook} (also known as a **data dictionary**) that explicitly documents what each variable is. [Figure @fig-management-mb-codebook] shows an example codebook for the trial-level data in the bottom of @fig-management-mb-datafiles. Each row represents one variable in the associated dataset. Codebooks often describe what type of variable a column is (e.g., numeric, string), and what values can appear in that column. A human-readable explanation is often given as well, providing units (e.g., "seconds") and a translation of numeric codes (e.g., "test condition is coded as 1").
...
* **Interoperability** means that research products needs to be in a format that people and machines (e.g., search engines and analysis software) can understand. 
* **Reusable** means that research products need to be well organized, documented, and licensed so that others know how to use them.
