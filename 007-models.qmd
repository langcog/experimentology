...
small_tea_data <- bind_rows(slice_head(tea_data, n = 6),
                            slice_tail(tea_data, n = 6))
```
...
ggplot(tea_data_means, aes(x = condition, y = rating)) +
  ...
  geom_text(aes(x = x, y = mean, label = label), data = tea_data_coefs,
            color = pal$blue, hjust = 0, parse = TRUE) +
...
ggplot(tea_data, aes(x = condition, y = rating)) + 
  ...
  stat_summary(fun.data = "mean_cl_boot", color = pal$red) + 
  labs(x = "Condition", y = "Rating")
```
...
![Decreases in estimation error due to adjusting for covariates, plotted by the N participants in each group and the correlation between the outcome (X) and the covariate (Z).](007-models_files/figure-html/models-precision-1.png){#fig-models-precision width=70%}
