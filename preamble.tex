...
% define a custom footnote for URLs
\usepackage{manyfoot}
%\usepackage[perpage]{manyfoot}
\DeclareNewFootnote{URL}[alph]

% for textual links, put URL into a footnote
\let\oldhref\href % save normal href
\renewcommand{\href}[2]{%
  \oldhref{#1}{#2}%keep link as is
  \footnoteURL{\url{#1}}%and add footnote for URL
}

% links within footnotes need footnotetext to be outside of footnote
\let\oldfootnote\footnote
\renewcommand\footnote[1]{{%
  \renewcommand\href[2]{%
    \global\def\cururl{##1}%
    \oldhref{##1}{##2}\footnotemarkURL\ }%
  \oldfootnote{#1}%
  \ifdefined\cururl%
    \footnotetextURL{\url{\cururl}}%
  \fi}}


\usepackage{lipsum} % for testing/debugging
