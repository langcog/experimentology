...
[NC: Personally, I think it helps present problems with focusing on significance]

Unfortunately, five subsequent studies testing whether social norm messages increased hotel towel reuse all failed to reject the null hypothesis (all $p$s $> .05$). You might conclude that these studies indicate that social norm messages do not reliably impact towel reuse. However, when all the evidence was combined in a meta-analysis, @scheibehenne2016 found that the social norm messages did have a significant overall effect on hotel towel reuse. Most of the studies, however, did not have large enough samples for this difference to be statistically significant. [MM: If we include effect sizes above, here we could allude to them again: "notice how the point estimates are large, even though the p-values are >0.05".] [MM: Suggest adding pooled estimate, CI, and p-value to model proper reporting]. Although each study was likely under-powered to detect the true effect, combining them improved power and ultimately suggested that social norm messaging does make a difference. 
:::
