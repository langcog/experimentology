...
```{r inference-null-model2, fig.cap="Example distribution of treatment effects under the null model for a larger experiment."}
...
```{r inference-bf, fig.cap="The Bayes Factor (BF)."}
...
As we've seen above, hypothesis testing hinges on approximating the null distribution of the statistical estimate. In the examples above, it was easy to use statistical theory to work out the null distribution (using our knowlege of the binomial or normal distribution). But sometimes we don't know what the null distribution would look like. Consider a case in which the ratings data from our tea-tasting experiment was very skewed (Figure (\@ref(fig:inference-skewed))).  
...
We can't proceed with a t-test in good conscience because, with only $n=18$, we can't necessarily trust that the Central Limit Theorem has "kicked in" sufficiently for the test to work despite the skewness. Stated otherwise, we can't be sure that the null distribution is normal (Gaussian) in this case.
...
The idea is that, if the treatment truly had no effect on the outcome, then the observations would be **exchangeable** between the treatment and control groups. That is, there would not be systematic differences between the treatment and control groups. This may or may not be true in our observed sample (after all, that's why we're doing a hypothesis test in the first place), but we could draw new samples from our existing sample in a manner that forces exchangability. 

In this case, we could randomly permute the column of ratings in our dataset while leaving the column of conditions fixed. If we do this hundreds or thousands of times and compute the treatment effect in each case, we create a null distribution over what we might expect the treatment effect to look like if there was no condition effect. We can then compare our *actual* treatment effect to this nonparametric null distribution. If it is smaller than the 2.5th percentile, or larger than the 97.5th percentile, we can reject the null with $p < .05$, just the same as if we had used a parametric test like a $t$-test.
:::
