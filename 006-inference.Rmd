...
```{r}
knitr::kable(head(tea_data), booktabs = TRUE, digits = 2)
```
:::
...
expand_grid(N = c(12,24,48,96), 
  ...
  kable(digits = 3, booktabs = TRUE, 
        align = "rrrr", caption = "Comparison of p-value and BF for several different (randomly-generated) tea-tasting scenarios.", table.envir = "margintable")
```
...
In practice, confidence intervals are often misinterpreted by students and researchers alike [@hoekstra2014]. In Hoekstra et al.'s example, a researcher conducts an experiment and reports "The 95% confidence interval for the mean ranges from 0.1 to 0.4". But then all of the statements in Table \@ref(tab:inference-ci-false), though tempting to make about this situation, are *technically false*.
...
kable(ci, table.envir = "table*", booktabs=TRUE,
