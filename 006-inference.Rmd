...
We've been arguing that experiments are about measuring effects. The effects we are interested in are causal effects for a group of people, but that group is almost always bigger than the participants in an experiment. **Statistical inference** is the process of going beyond the specific characteristics of the sample that you measured to make generalizations about the broader **population**.  Chapter \@ref(estimation) already showed us how to make one simple inference: estimating population parameters using both frequentist and Bayesian techniques.
...
Next, we just use the `group_by` and `summarise` workflow from Appendix \@ref(tidyverse) to get the estimated treatment effect for each of these simulations.
...
This tibble gives us what we would need to plot the sampling distributions above in Figures \@ref(fig:inference-sampling-small) and \@ref(fig:inference-sampling-big).
:::
...
But look what happens as we draw increasingly larger samples in Figure \@ref(fig:inference-coin-ns): We get a normal distribution! This tendency of sampling distributions to become normal as $n$ becomes very large reflects a deep and elegant mathematical law called the **Central Limit Theorem**. 
...
Let's take a look at what the null model might look like. We already tried out repeating our tea-tasting experiment thousands of times in our discussion of sampling above. Now in Figure \@ref(fig:inference-null-model), we do the same thing but we assume that the **null hypothesis** of no treatment effect is true. The plot shows the distribution of treatment effects $\hat{\beta}$ we observe: some a little negative, some a little positive, and a few substantially positive or negative, but mostly zero. 
...
```{r inference-null-model, fig.cap="One example of the distribution of treatment effects under the null model (with N=9 per group). The red regions indicate the part of the distribution in  which less than 5\\% of observations should fall."}
...
Let's compare $p$ values and Bayes Factors (computed using the default setup in the `BayesFactor` R package). In Table \@ref(tab:p-bf-comparison), the rows represent simulated experiments with varying total numbers of participants (N) and varying average treatment effects. Both $p$ and BF go up with more participants and larger effects. In general, BFs tend to be a bit more conservative than $p$-values, such that $p<.05$ can sometimes translate to a BF of less than 3 [@benjamin2018]. For example, take a look at the row with 48 participants and an effect size of 1: the $p$ value is less than .05, but the Bayes Factor is only 2.0. 
  ...
  summarise(`p value` = printp(t.test(x = rating[condition == "milk first"],
         ...
         BF = round(extractBF(
           BayesFactor::ttestBF(x = rating[condition == "milk first"],
                                y = rating[condition == "tea first"], 
                                paired = FALSE))$bf,1)) %>%
  kable(digits = 3, align = "rrrr", caption = "Comparison of p-value and BF for several different (randomly-generated) tea-tasting scenarios.", table.envir = "margintable")
```
...
Beyond these misconceptions, there's another problem. The $p$-value is a probability of a certain set of events happening (corresponding to the observed data or any "more extreme" data, that is to say, data further from the null). Since $p$-values are probabilities, we can combine them together across different events. If we run a null experiment -- an experiment where the expected effect [MM: "true effect"? "Null experiment" seems a bit confusing since it could also refer to the hypothesis-testing result.] is zero -- the probability of a dataset with $p < .05$ is of course .05. But if we run two such experiments, we can get $p < .05$ with probability `r round(2*.05 - .05*.05,2)`. By the time we run 20 experiments, we have an `r round(1 - .95^20,2)` chance of getting a positive result.
...
```{r inference-condition-cis, fig.cap = "Confidence intervals on each of the two condition estimates, as well as on the difference between conditions."}
...
In practice, confidence intervals are often misinterpreted by students and researchers alike [@hoekstra2014]. In Hoekstra et al.'s example, a researcher conducts an experiments and reports "The 95% confidence interval for the mean ranges from 0.1 to 0.4". [MM: Huh? What's wrong with this one? It's not very informative, but it's not a misinterpretation. Maybe just remove this sentence?] All of the statements in Table \@ref(tab:inference-ci-false), though tempting to make, are *technically false*.
...
2. Take three of Goodman's (2008) "dirty dozen" in Table \@ref(tab:dirty-dozen) and write a description of why each is a misconception. (These can be checked against the original article, which gives a nice discussion of each). 
:::
