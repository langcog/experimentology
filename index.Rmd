...
```{r}
# hooks to reformat latex output
if (knitr::is_latex_output()) {
  ...
  knitr::knit_hooks$set(
    ...
    chunk = function(x, options) {
      if (stringr::str_detect(x, "\\\\begin\\{table\\*?\\}")) {
        message(x)
        # put label after caption (instead of label nested within caption)
        x <- x |>
          # variant for \caption{(\#tab:LABEL)CAPTION}
          str_replace(regex("(\\\\begin\\{table\\*?\\}.*\\\\caption)\\{(\\(\\\\#tab.*?\\))(.*)\\}",
                            dotall = TRUE),
                      "\\1\\{\\3\\}\\2") |>
          # variant for \caption{\label{tab:LABEL}CAPTION}
          str_replace(regex("(\\\\begin\\{table\\*?\\}.*\\\\caption)\\{(\\\\label\\{.*?\\})(.*)\\}",
                            dotall = TRUE),
                      "\\1\\{\\3\\}\\2")
        message(x)
      }
  ...
  knitr::knit_hooks$set(
    ...
    plot = function(x, options) {
      # first apply default hook
      x <- default_plot_hook(x, options)
      # message(x)
      if (!is.null(options$boxfigure)) {
        message(x)
        # replace \caption[text]{text} -> \captionof{figure}{text}
        x <- x |>
          str_replace("\\\\caption\\[.*\\]\\{(.*)\\}",
                               "\\\\captionof\\{figure\\}\\{\\1\\}") |>
          # override figure or marginfigure environment with boxfigure environment
          str_replace("\\\\begin\\{.*\\}", "\\\\begin\\{boxfigure\\}") |>
          str_replace("\\\\end\\{.*\\}", "\\\\end\\{boxfigure\\}")
      }
      return(x)
    })
...
This observation leads to our book's title. **Experimentology** is not psychology, cognitive science, or any other body of content knowledge -- but rather the set of practices, findings, and approaches that enable the construction of robust, precise, and generalizable experiments. 
...
Even for students who are working on non-experimental research, we expect that a substantial part of the book content will still be useful, including chapters on replication (Chapter \@ref(replication)), ethics (Chapter \@ref(ethics)), sampling (Chapter \@ref(sampling)), project management (Chapter \@ref(management)), and all of the material on reporting (Chapters \@ref(writing) - \@ref(meta)).
...
* **Bias reduction**: A precise estimate is not useful if it is biased away from the true quantity. In our samples, analyses, experimental designs, and in the literature, we need to think carefully about sources of bias in the quantity being estimated. 
...
* The `RMarkdown` and `Quarto` tools for creating reproducible reports that can be rendered to a variety of formats (Appendix \@ref(rmarkdown)),
...
\noindent Where appropriate, we provide **code boxes** that show the specific `R` code used to create our examples. 
