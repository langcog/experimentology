...
syllabi <- readr::read_csv("data/instructor_guide/Content_Skills_10wk.csv") 
color.me <- which(syllabi$Difficulty == "adv")

syllabi |>
  mutate(Chapter = stringr::str_replace(Chapter, "Chapter ", ""), 
         Chapter = ifelse(!is.na(Chapter), Chapter, "")) |>
  mutate(Appendix = stringr::str_replace(Appendix, "Appendix ", ""), 
         Appendix = ifelse(!is.na(Appendix), Appendix, "")) |>
  select(-Difficulty) #|>
  # kable() |> 
  # row_spec(color.me, bold = F, color = "black", background = "#ebe8fc")
```
...
syllabi <- readr::read_csv("data/instructor_guide/Content_10wk.csv") 
color.me <- which(syllabi$Difficulty == "adv")
...
syllabi <- readr::read_csv("data/instructor_guide/Content_Skills_16wk.csv") 
color.me <- which(syllabi$Difficulty == "adv")
...
syllabi <- readr::read_csv("data/instructor_guide/Content_16wk.csv") 
color.me <- which(syllabi$Difficulty == "adv")
