...
Div = function (div)
  ...
  if #div.attr.classes ~= 0 and div.classes[1] == "box" then
    options = ''
  end
  ...
  local boxtitle = ''
  if class == "box" then
    env = div.attributes['data-box']
    if not env then
      print('Error: box type not supplied for box with title:', div.attributes['data-title'])
    end
    title = div.attributes['data-title']
    if title then
      boxtitle = '\n\\boxtitle' .. '{' .. title .. '}'
    end
  else
  ...
  local beginEnv = '\\begin' .. '{' .. env .. '}' .. options .. boxtitle
  local endEnv = '\n\\end{' .. env .. '}'
