...
As psychology moves into an era of bigger data and more complex behaviors, we become increasingly reliant on **data visualization literacy** [@borner2019data] to make sense of what is going on. Further, as a researcher reporting about your data, creating appropriate visualizations that are aligned with your analyses (as well as your design and preregistration) is an important part of [transparency]{.smallcaps} and [bias reduction]{.smallcaps} in your reporting. 
...
![Mapping out a cholera epidemic (1854). Line shows region for which Broad Street pump is nearest.](images/viz/snow_cholera_voronoi.png){#fig-viz-cholera width="45%"}
...
![Unlike statistical visualization, which aims to clearly expose the logic of an experiment at a glance, infoviz aims to provide a rich world of patterns to explore [reproduced from @infoviz].](images/viz/viz_infoviz.png){#fig-viz-infoviz .column-margin}
...
Analogous to the "default" or "saturated" model in @sec-models, the design plot should show the key dependent variable of the experiment, broken down by all of the key manipulations.
...
Both of these steps are the visual analogue of p-hacking! Instead, the design plot is the "preregistered analysis" of your visualization: it illustrates a first look at the estimated causal effects from your experimental manipulations. In the words of @coppock2019, "visualize as you randomize"!
...
![Principles of visual perception can help guide visualization choices. Reproduced from @mackinlay1986 (see also @cleveland1984).](images/viz/viz_hierarchy.png){#fig-viz-hierarchy .column-margin}
...
It is much easier to compare the slopes of two lines than mentally compute a difference of differences between bars.
Here are a few corollaries of this principle (adapted from [a presentation by Karl Broman](https://www.biostat.wisc.edu/~kbroman/presentations/IowaState2013/graphs_combined.pdf)).
...
While these analytic equations remain common, an increasingly popular alternative is to *bootstrap* confidence intervals (see Depth box in @sec-inference for more on bootstrapping).
...
![This figure uses complicated 3D ribbons to compare distributions across four countries (from @roeder1994dna). How could the same data have been presented more legibly?](images/viz/roeder_badviz.png){#fig-viz-bad2 .column-margin width="90%"}
