...
```{r viz-cholera, boxfigure=TRUE}
#| label: fig-viz-cholera
#| fig-cap: "Mapping out a cholera epidemic (1854). Line shows region for which Broad Street pump is nearest."
...
```{r viz-covid, boxfigure=TRUE}
#| label: fig-viz-covid
#| fig-cap: "Map showing the known locations of cumulative coronavirus cases by share of the population in each county (reproduced from the New York Times)."
...
```{r viz-cases, boxfigure=TRUE}
#| label: fig-viz-cases
#| fig-cap: "7-day average of new reported COVID cases (reproduced from the New York Times)."
...
```{r viz-cases2, boxfigure=TRUE}
#| label: fig-viz-cases2
#| fig-cap: "Rates of COVID cases by vaccination status (reproduced from [`covid.cdc.gov`](https://covid.cdc.gov/covid-data-tracker/#rates-by-vaccine-status))."
...
```{r viz-infoviz}
#| label: fig-viz-infoviz
#| fig-cap: "Unlike statistical visualization, which aims to clearly expose the logic of an experiment at a glance, infoviz aims to provide a rich world of patterns to explore [reproduced from @infoviz]."
#| column: margin
...
```{r viz-hierarchy}
#| label: fig-viz-hierarchy
#| fig-cap: "Principles of visual perception can help guide visualization choices. Reproduced from @mackinlay1986automating (see also @cleveland1984graphical)."
#| column: margin
...
```{r viz-heatmap, boxfigure=TRUE}
#| label: fig-viz-heatmap
#| fig-cap: "Heatmap showing a specific range of continuous parameters where an effect emerged [reproduced from @barnett2022]."
...
Or as sizes/areas (@fig-viz-prepost3).
...
```{r viz-boot, boxfigure=TRUE}
#| label: fig-viz-boot
#| fig-cap: "Three different error bars for the Stiller data: bootstrapped 95\\% confidence intervals (left), standard error of the mean (middle), and analytically computed confidence intervals (right)."
...
```{r viz-bad}
#| label: fig-viz-bad
#| fig-cap: "This figure uses a lot to ink to show exactly three numbers, for a \"ddi\" of $0.2$ [This came from the Washington Post, 1978; see @wainer1984display for other examples]."
#| column: margin
...
```{r viz-bad2}
#| label: fig-viz-bad2
#| fig-cap: "This figure uses complicated 3D ribbons to compare distributions across four countries [from @roeder1994dna]. How could the same data have been presented more legibly?"
#| column: margin
...
```{r viz-gorilla, boxfigure=TRUE}
#| label: fig-viz-gorilla
#| fig-cap: "A dataset was constructed by @yanai2020 which revealed a picture of a gorilla when the raw data were plotted."
...
```{r viz-uniform, boxfigure=TRUE}
#| label: fig-viz-uniform
#| fig-cap: "A suspiciously uniform distribution abruptly cutting off at 50k miles. Ring the alarm!"
...
Are ages roughly similar across each country (@fig-viz-blake5)?
