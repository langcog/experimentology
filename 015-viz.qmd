...
As psychology moves into an era of bigger data and more complex behaviors, we become increasingly reliant on **data visualization literacy**\index{data visualization literacy} [@borner2019data] to make sense of what is going on. Further, as a researcher reporting about your data, creating appropriate visualizations that are aligned with your analyses (as well as your design and preregistration) is an important part of [transparency]{.smallcaps}\index{transparency} and [bias reduction]{.smallcaps}\index{bias reduction} in your reporting. 
...
In a design where the number of trials was carefully controlled, seeing unexpected numbers here (like the 14 or 15 trial bins) are clues that something else may be going on in the data. In this case, it was a small number of trials with missing data.\index{missing data}
...
Here, however, we show the linear regression\index{linear regression} trend, `geom_smooth(method = "lm")`, which better corresponds to the predictions of the study and the statistical model being used (see @sec-models).
Other regression forms can be specified with the `formula` argument.
For example, we could show quadratic\index{quadratic effect} smoothing with `geom_smooth(method = "lm", formula = y ~ poly(x, 2))`.
