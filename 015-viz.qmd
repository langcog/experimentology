...
![Mapping out a cholera epidemic (1854). Line shows region for which Broad Street pump is nearest.](images/viz/snow_cholera_voronoi.jpeg){#fig-viz-cholera}
...
![Map showing the known locations of cumulative coronavirus cases by share of the population in each county (reproduced from the New York Times).](images/viz/covid-hot-spots.png){#fig-viz-covid}
...
![7-day average of new reported COVID cases (reproduced from the New York Times).](images/viz/covid-cases.png){#fig-viz-cases}
...
![Rates of COVID cases by vaccination status (reproduced from [`covid.cdc.gov`](https://covid.cdc.gov/covid-data-tracker/#rates-by-vaccine-status)).](images/viz/vaccination.png){#fig-viz-cases2}
...
![Heatmap showing a specific range of continuous parameters where an effect emerged (reproduced from @barnett2022).](images/viz/viz_heatmap_example.png){#fig-viz-heatmap}
:::
...
![Three different error bars for the Stiller data: bootstrapped 95\\% confidence intervals (left), standard error of the mean (middle), and analytically computed confidence intervals (right).](015-viz_files/figure-html/fig-viz-boot-1.png){#fig-viz-boot}
...
![A dataset was constructed by @yanai2020 which revealed a picture of a gorilla when the raw data were plotted.](images/viz/gorilla.png){#fig-viz-gorilla}
...
![A suspiciously uniform distribution abruptly cutting off at 50k miles. Ring the alarm!](images/viz/data_colada_uniform.png){#fig-viz-uniform}
