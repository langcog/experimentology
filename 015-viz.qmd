...
![Rates of COVID cases by vaccination status (reproduced from [`covid.cdc.gov`](https://covid.cdc.gov/covid-data-tracker/#rates-by-vaccine-status)).](images/viz/vaccination.png){#fig-viz-cases2 width="70%"}
...
The visualization library `ggplot` (see @sec-ggplot) makes the mapping of variables in the data to visual data. The first part of a ggplot call is an `aesthetics` layer:
...
```{r viz-prepost1}
d <- readr::read_csv("https://raw.githubusercontent.com/langcog/experimentology/main/data/tidyverse/stiller_scales_data.csv") |>
...
```{r viz-prepost2}
#| label: fig-viz-prepost2
#| fig-cap: "A first visualization of the Stiller et al. (2015) data."
...
```{r viz-prepost3}
#| label: fig-viz-prepost3
#| fig-cap: "Iterating on the Stiller data."
#| fig-cap-location: margin
...
@fig-viz-show-data1 shows the Stiller data with error bars. 
<!-- As such, they allow the viewer to interpret the means as *estimates*.  -->

```{r viz-show-data1}
#| label: fig-viz-show-data1
...
```{r eval=FALSE, echo=TRUE}
ggplot(ms, 
       aes(x = age_group, y = empirical_stat,
...
```{r viz-raincloud}
#| label: fig-viz-raincloud
#| fig-cap: "Example of a raincloud plot, reproduced from @allen2019raincloud."
#| fig-cap-location: margin
...
![Three different error bars for the Stiller data: bootstrapped 95% confidence intervals (left), standard error of the mean (middle), and analytically computed confidence intervals (right).](015-viz_files/figure-pdf/fig-viz-boot-1.png){#fig-viz-boot width="80%"}
...
ggplot(ms, aes(x = age_group, y = empirical_stat, 
  ...
  geom_point() +
  geom_line() +
  ggrepel::geom_text_repel(aes(label = condition, colour = condition), 
                           data = ms |> filter(age_group == "(3,4]"), 
                           nudge_y = .1, nudge_x = .5, segment.color = NA) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0) +
  geom_jitter(aes(x = age_group, y = rating, color = condition, fill = condition),
              alpha = 0.25, width = 0.05, data = mss) +
  labs(x = "Age Group", y = "Mean Accuracy") +
  scale_colour_few() +
  # theme_few(base_size = 10) +
  theme_few() +
...
![Anscombe's quartet [@anscombe1973graphs].](images/viz/viz_anscombe.png){#fig-viz-anscombe .margin-caption}
...
@fig-viz-datasaurus shows just how bad things can get when we rely on summary statistics. 
...
![Originally inspired by a figure constructed by [@datasaurus](http://www.thefunctionalart.com/2016/08/download-datasaurus-never-trust-summary.html) using the [drawMyData](http://robertgrantstats.co.uk/drawmydata.html) tool, we can actually construct an arbitrary number of different graphs with exactly the same statistics [@matejka2017same;@murray2021generating]. This set, known as the [The Datasaurus Dozen](https://www.autodesk.com/research/publications/same-stats-different-graphs), even has the same set of boxplots.](images/viz/datasaurus.png){#fig-viz-datasaurus .margin-caption}
...
![A dataset was constructed by @yanai2020 which revealed a picture of a gorilla when the raw data were plotted.](images/viz/gorilla.png){#fig-viz-gorilla width="50%"}
...
For example, when @datacolada made a simple histogram of the car mileage data reported in @shu2012signing [released publicly by @kristal2020signing], they were immediately able to observe that it followed a perfectly uniform distribution, truncated at exactly 50,000 miles (@fig-viz-uniform).
...
![A suspiciously uniform distribution abruptly cutting off at 50k miles. Ring the alarm!](images/viz/data_colada_uniform.png){#fig-viz-uniform width="70%"}
...
fairness <- readr::read_csv(paste0(repo, "data/viz/ontogeny_of_fairness.csv")) |>
...
```{r viz-blake1}
d <- readr::read_csv("data/viz/ontogeny_of_fairness.csv") |>
  mutate(trial_num = as.numeric(gsub(".(\\d+)", "\\1", trial)),
         trial_type = factor(eq.uneq,
                             levels = c("E","U"),
                             labels = c("Equal","Unequal")),
