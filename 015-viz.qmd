...
As psychology moves into an era of bigger data and more complex behaviors, we become increasingly reliant on **data visualization literacy**\index{data visualization literacy} [@borner2019data] to make sense of what is going on. Further, as a researcher reporting about your data, creating appropriate visualizations that are aligned with your analyses (as well as your design and preregistration\index{preregistration}) is an important part of [transparency]{.smallcaps}\index{transparency} and [bias reduction]{.smallcaps}\index{bias reduction} in your reporting. 
...
An obstetrician and anesthesiologist named John Snow,\index{Snow, John} however, had proposed an alternative theory: rather than spreading through foul air, he thought that cholera\index{cholera} was spreading through a polluted water supply [@snow1855mode].\index{Snow, John}
...
Furthermore, a line could be drawn representing the region that was closest to each water pump, a technique that is now known as a Voronoi diagram (<https://en.wikipedia.org/wiki/Voronoi_diagram>).\index{Voronoi diagram}
...
![Weekly counts of new reported COVID\index{COVID-19 pandemic} hospital admissions in the US between August 2020 and January 2024 (from the CDC\index{Centers for Disease Control and Prevention (CDC)} COVID Data Tracker, <https://covid.cdc.gov/covid-data-tracker>). _Usage does not constitute endorsement or recommendation by the US Government, Department of Health and Human Services, or Centers for Disease Control and Prevention._](images/viz/cdc-covid-trend.png){#fig-viz-cases width="60%" fig-alt="A plot of weekly COVID new hospital admissions over time (8/2020 to 1/2024), with several waves where hospitalizations peak."}
...
The art of designing and producing these graphics is typically known as **infoviz**\index{infoviz} and should be distinguished from what we call **statistical visualization**\index{statistical visualization} [@gelman2013].
...
Critically, design plots\index{design plot} should neither omit particular manipulations because they didn't yield an effect nor include extra covariates because they seemed interesting after looking at the data.
Both of these steps are the visual analogue of $p$-hacking! Instead, the design plot\index{design plot} is the "preregistered analysis" of your visualization: it illustrates a first look at the estimated causal effects from your experimental manipulations. In the words of @coppock2019, "Visualize as You Randomize"!

It can sometimes be a challenge to represent the full pattern of manipulations from an experiment in a single plot. Below we give some tricks for maximizing the legible information in your plot. But if you have tried these and your design plot\index{design plot} still looks crowded and messy, that could be an indication that your experiment is manipulating too many things at once!
...
Other critical variables of interest (e.g., secondary manipulations and demographics) are then assigned to "visual variables" (e.g., color, shape, or size).
...
![Principles of visual perception can help guide visualization choices. Based on @mackinlay1986 [see also @cleveland1984].](images/viz/viz_hierarchy.png){#fig-viz-hierarchy .column-margin fig-alt="A diagram of visual features listed from more to less accurate: position, length, angle/slope, area, volume, color/density."}
...
Lines then extend out to the biggest and smallest values (excluding outliers, which are shown as dots). [Figure @fig-viz-show-data2] gives the boxplots for the Stiller data, which don't look that informative---perhaps because of the coarseness of individual participant averages due to the small number of trials. 
...
![This figure uses a lot to ink to show three numbers, for a "ddi" of 0.2 (from the *Washington Post*, 1978; see @wainer1984display for other examples).](images/viz/bad-viz1.png){#fig-viz-bad .column-margin width="90%" fig-alt="A plot showing labeled 3D bars over 3 years; they have a Japanese flag pattern and are in front of a American flag backdrop."}
...
![This figure uses complicated 3D ribbons to compare distributions across four countries [from @roeder1994dna]. How could the same data have been presented more legibly?](images/viz/roeder_badviz.png){#fig-viz-bad2 .column-margin width="90%" fig-alt="2 plots showing binned frequency data for 4 countries in using 3D ribbons in slightly different shades of gray."}
...
For example: consider whether the visual complexity introduced by the default grey background and grid lines in @fig-viz-themes0 is justified, or whether a more minimal theme would be sufficient.^[See the `ggthemes` package [@arnold2023] for a good collection of themes.]
...
```{r tbl-datasaurus}
#| tbl-cap: Summary statistics for each dataset in the Datasaurus Dozen\index{Datasaurus} (Matejka 2017).
...
::: {.content-visible when-format="html"}
![](015-viz_files/figure-html/fig-viz-gorilla-1.png){width=50% fig-alt="A scatterplot of Body Mass Index against Steps with different colors corresponding to Male and Female; looks like a gorilla."}
:::
...
::: {.content-visible when-format="html"}
![](015-viz_files/figure-html/fig-viz-uniform-1.png){width="70%" fig-alt="A histogram showing counts of implied miles driven; there are no bars above 50000 implied miles driven."}
:::
...
Finally, we can make a few versions of the design plot\index{design plot} that are broken out by different variables.
...
@Fig-viz-blake10 is now looking much closer to a quick-and-dirty version of a "design plot"\index{design plot} we might include in a paper.
...
Bugs that are subtle and invisible in other parts of the analysis pipeline\index{analysis pipeline} will often pop out as red flags in visualizations.
