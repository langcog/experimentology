...
Nearly two centuries later, as the COVID-19 pandemic swept through the world, governmental agencies like the CDC produced maps of the outbreak that became much more familiar (@fig-viz-covid). 

![Map showing the known locations of cumulative coronavirus cases by share of the population in each county (reproduced from the New York Times).](images/viz/covid-hot-spots.png){#fig-viz-covid width="55%"}
...
![7-day average of new reported COVID cases (reproduced from the New York Times).](images/viz/covid-cases.png){#fig-viz-cases width="65%"}
...
![Rates of COVID cases by vaccination status (reproduced from <https://covid.cdc.gov/covid-data-tracker>).](images/viz/vaccination.png){#fig-viz-cases2 width="60%"}
...
![Heatmap showing a specific range of continuous parameters where an effect emerged [reproduced from @barnett2022].](images/viz/viz_heatmap_example.png){#fig-viz-heatmap width="35%"}
:::
...
For example, if you're working with a skewed response variable or a dataset with clear outliers, and you want to estimate medians and quartiles.  
...
![](015-viz_files/figure-html/fig-viz-boot-1.png){width="80%"}
:::
...
```{r tbl-datasaurus}
#| tbl-cap: Summary statistics for each dataset in the Datasaurus Dozen (Matejka 2017).
