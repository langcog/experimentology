...
We'll try to go easy on notation, but some amount will hopefully make things clearer. We'll use $\theta_{\textrm{M}}$ ("theta") to denote the parameter we want to estimate (the population parameter\indexC{population parameter}) and $\widehat{\theta}_{\textrm{M}}$ for **sample estimate**\indexC{sample estimate}.^[Statisticians use "hats" like this to denote estimates from a specific sample. One way to remember this is that the "person in the hat" is wearing a hat to dress up as the actual quantity.]
...
How do we quantify this trade-off between our prior expectations and our current observations? We can do this via **Bayesian estimation**\indexC{Bayesian estimation} of $\widehat{\theta}_{\textrm{M}}$. Bayesian estimation provides a principled framework for integrating prior beliefs and data. These estimation techniques can be very helpful in cases where data are sparse or prior beliefs are strong.
