...
A model like this is called a **linear regression of Y (the outcome) on X (the predictor variable)**. In the first line, $\theta_{milkfirst} + \Delta X_i$ is called the **linear predictor**, and it describes the expected value of an individual's tea rating, $Y_i$, given that person's treatment group $X_i$ (the single predictor variable in this model). That is, for a participant in the control group, the linear predictor is just equal to $\theta_{milkfirst}$, which is indeed the mean for the control group. On the other hand, for a participant in the treatment group, the linear predictor is equal to $\theta_{milkfirst} + \Delta$, which is indeed the mean for the treatment group. In regression jargon, $\Delta$ is a **regression coefficient**, representing the association of the predictor variable $X_i$ with the outcome $Y_i$.

The terms $\epsilon_i$ are **error terms**, referring to random variation of participants' ratings around their group mean. Note that this is a very specific kind of "error"; it not to "error" due to bias, for example. Instead, you can think of the error terms as capturing the "error" that would be associated with predicting any given participant's rating based on just the linear predictor. If you thus predicted a control group participant's rating as $\theta_{milkfirst}$, this would be a good guess, but you still expect the participant's rating to deviate somewhat from $\theta_{milkfirst}$ due to "error". In this model, the linear predictor and error terms together say that participants' ratings scatter randomly (actually, normally) around their group means with standard deviation $\sigma$. And that is exactly the model we posited earlier. 

Having posited this model, we then want to estimate the regression coefficients. The usual method to do so is called **ordinary least squares**. Here is the basic idea. For any given regression coefficient estimates $\widehat{\theta}_{milkfirst}$ and $\widehat{\Delta}$, we would get different **predicted values**, $\widehat{Y}_i = \theta_{milkfirst} + \Delta X_i$ for each participant. **The goal of OLS estimation is to find the values of the regression coefficient that optimize these predictions, i.e., brings them as close as possible to participants' true outcomes, $Y_i$.**[^models-1] 


[^models-1]: Specifically, OLS minimizes squared error loss, in the sense that it will choose the regression coefficients whose predictions minimize $\sum_{i=1}^n \left( Y_i - \widehat{Y}_i\right)^2$, where $n$ is the sample size. A wonderful thing about OLS is that those optimal regression coefficients (\widehat{\mathbf{\beta}}) turn out to have a very simple closed form: $\widehat{\mathbf{\beta}} = \left( \mathbf{X}'\mathbf{X} \right)^{-1} \mathbf{X}'\mathbf{y}$. Because there could be multiple predictors, $\widehat{\mathbf{\beta}}$ is a vector, $\mathbf{X}$ is a matrix of predictor variables for each subject, and $\mathbf{y}$ is a vector of participants' outcomes. As more good news, the standard error for $\widehat{\mathbf{\beta}}$ has a similarly simple closed form. (MBM to MCF: Feel free to remove this footnote if it's too much.)


## Generalizing the linear predictor: Adding covariates


It turns out that much more flexible statistical models can also be written as regression models, similarly to what we did for the simple tea-tasting experiment via Equation (\ref{eq:ols_ttest}). For example, we might want to add another predictor variable, such as age. [USE EXISTING DYLAN EXAMPLE BELOW?] We could then add the new predictor variable and a corresponding regression coefficient to the model:
...
Now that we have multiple predictor variables, we've labeled them $X_{i1}$ (treatment group) and $X_{i2}$ (age in years). 

To illustrate how to interpret the regression coefficients in this model, let's use the linear predictor to compare the predictions for two hypothetical participants who are both in the treatment group: 20-year-old Alice and 21-year old Bob. Alice's linear predictor tells us that her expected rating is $\theta_{milkfirst} + \Delta + \theta_{age} \cdot 20$. In contrast, Bob's linear predictor is $\theta_{milkfirst} + \Delta + \theta_{age} \cdot 21$. We could therefore calculate the expected difference in ratings for 30-year-olds versus 20-year olds by subtracting Alice's linear predictor from Bob's, yielding just $\theta_{age}$. How simple! In fact, we would get the same result if Alice and Bob were instead 50 and 51 years old, respectively (try this yourself). This illustrates a key point about linear regression models in general: **the regression coefficient represents the average (i.e., expected) difference in $Y_i$ when comparing participants who differ by 1 unit of the corresponding predictor variable** (here, comparing participants who differ by 1 year of age). In "Practical modeling considerations" below, we discuss whether and when to control for covariates.

## When does linear regression work?

* Talk about diagnostics

The models shown above (Equations \ref{eq:ols_ttest} and \ref{eq:ols_one_covariate}) are a special type of linear regression model called **ordinary least squares (OLS)**. These models are linear in the sense that they assume that expected outcomes scale linearly as any given predictor variable increases or decreases (e.g., a 1-unit increase in age is associated with a constant change in the expected outcome). OLS also assumes, critically, that the error terms ($\epsilon_i$) have the same normal distribution (i.e., mean 0 and variance $\sigma^2)$) for *any* fixed values of the predictor variables. In statistical jargon, we assume the errors are independent and identically distributed and homoskedastic. 

OLS is incredibly powerful. In fact, if the assumptions above are fulfilled, OLS is the best we can possibly do.[^models-2] However, it's important to be aware of situations where its assumptions might be violated. The most common such situations are: 

[SHOULD STATE THE ASSUMPTIONS FIRST IN POSITIVE TERMS; COULD MAYBE TURN SOME OF THIS INTO A TABLE?]

1. **The linear predictor is misspecified.** Notice that in our comparison of Alice's and Bob's expected outcomes based on their 1-year age difference, we were able to interpret the coefficient $\theta_{age}$ as the average difference in $Y_i$ when comparing participants who differ by 1 year of age, *regardless* of whether those ages are 20 vs. 21, 50 vs. 51, etc. This is because the way the linear predictor is specified in Equation \ref{eq:ols_one_covariate} embeds the assumption that age is *linearly* related to expected outcomes. For many scientific contexts, this assumption is reasonable, but in other contexts, it may be badly violated. For example, if we sampled participants whose ages ranged from 1 to 60 years and plotted participants' ages versus their vocabulary sizes, we would probably find that there is a much bigger difference in vocabulary between typical 1-year-olds vs. typical 2-year olds than between typical 50-year-olds vs. typical 51-year olds. One way to accommodate such **nonlinearities** is to modify the linear predictor to include polynomial terms, such as $age^2$, which then allow us to fit curves (e.g., parabolas) rather than just lines. It is always a good idea to use visualizations like scatterplots to look for possible problems with the specification of the linear predictor. It can be a helpful visual aid to superimpose on these plots a model-free (i.e., "nonparametric") estimate of the relationship, such as a LOESS curve. 
...
2. **The errors are not independent.** Suppose we have repeated-measures data in which we sample participants, and then obtain multiple outcome measurements over time for each participant. Usually, outcome measurements within a participant will be correlated. For example, if we measure 10 participants' blood pressures every day for a week, some participants will typically have high values whereas others will typically have low values, even though there is some random variation within participants.[^models-3] To help identify such situations, it can be helpful to plot the distribution of residuals for each participant (or a random sample thereof) to see whether this distribution seems to differ across participants.

[YOU NEED TO EXPLAIN EARLIER WHAT RESIDUALS ARE]

[Suggested Figure. Violin plots, or similar, showing residuals for different subjects. The residuals exhibit non-independence.]

3. **The errors are not homoskedastic (i.e., they are heteroskedastic).** Heteroskedasticity means that the error terms are more variable for some values of the predictor variables than for other values. This is often the case for highly skewed outcome variables. For example, if we regressed personal income on years of education, we will find that higher education is associated with higher income. However, because income is highly right-skewed (some people have extremely high incomes), income is more variable for individuals with more education than for those with less education.[^models-3] For this reason, if we predicted incomes for individuals with 16 years of education (i.e., they completed 4-year college and then stopped) and also for individuals with 8 years (i.e., they completed middle school and then stopped), the errors in the former predictions will be much more variable than the errors in the latter predictions. It is a good idea to plot each predictor variable versus the residuals to see if the residuals are more variable for certain values of the predictor variable than for others.

[Suggested Figure. X vs. residuals, showing heteroskedasticity.]


[^models-2]: There is a precise sense in which OLS gives the *very best* predictions we could ever get from any model that posits linear relationships between the predictor variables and the outcome. That is, you can come up with any other linear model you want, and yet if the assumptions of OLS are fulfilled, predictions from OLS will always be less noisy than those of your model. This is because of an elegant mathematical result called the Gauss-Markov Theorem. (MBM to MCF: Feel free to delete.)

[^models-3] To be specific, it is not the blood pressure values ($Y_i$) themselves that must be independent, but rather the error terms ($\epsilon$). The error terms are what is "left over" after accounting for systematic variation that is predicted by the predictor variables. If, in principle, we managed to include in the linear predictor all variables that might explain individual differences in typical blood pressure (e.g., genetic factors, sex, age, etc.), then the errors would be independent even though the outcomes themselves are not. However, as a heuristic, considering possible sources of correlation in the outcomes themselves is often a reasonable proxy for thinking about the error terms.



## Generalizing the link function: Accommodating non-continuous outcomes

So far we have considered continuous outcome measures, like tea ratings. What if we instead had a binary outcome, such as whether a participant liked or didn't like the tea, or a count outcome, such as the number of cups a participant chose to drink? These and other non-continuous outcomes often violate the assumptions of OLS, in particular because they often induce heteroskedastic errors. For example, count outcomes are often right-skewed because a count cannot be less than 0, creating heteroskedasticity for the same reasons discussed above for skewed continuous outcomes. Binary outcomes inherently violate heteroskedasticity because the variance of a binary variable depends directly on the success probability, such that the errors will be more variable for individuals whose covariate values place their expected success probability closer to 0.50.[^models-4] 

Happily, there are regression models closely related to OLS that can handle non-continuous outcomes. These models are called **generalized linear models** (GLM), and OLS is one of many members of this large class of models. To see the connection, let's write an OLS model more generally in terms of what it says about the expected value of the outcome:

\begin{align}
\label{eq:ols_general_form}
E[Y_i] &= \beta_0 + \sum_{j=1}^p \beta_j X_j
\end{align}
where $p$ is the number of predictor variables, $\beta_0$ is the intercept, and $\beta_j$ is the regression coefficient for the $j^{th}$ predictor variable.
