...
On the other hand, sometimes there is no reference level for a factor. For example, in the @kovacs2010 paradigm, it's not clear whether a positive or negative belief is the reference level. That's not a problem in a fully-crossed design like theirs, but this situation can pose a problem because if you have more than two such factors. Ideally you would want to run independent experiments, but you have to choose some level for all of the other variables. One solution for three factors is do a Latin-square type design.^[There's a variant called the "Greco-Latin square" for four factors, in case you need that.] A Latin square is an $n x n$ matrix in which each number occurs exactly once in each row and column, e.g. $$\begin{bmatrix} 
	1 & 2 & 3 \\
	2 & 3 & 1\\
	3 & 1 & 2 \\
	\end{bmatrix}$$. This Latin square for $n=3$ gives the solution for a 3x3x3 experiment in which the row number is one factor, the column number is the second factor, and the number in that cell is the third factor. Although a fully-crossed design would require 27 cells to be run, the Latin square has only nine. Critically, the combinations of all each factor is balanced across the nine cells so that the average effect of each level of the three factors can be estimated.^[You can also check and see that no interactions can be estimated, because no factor co-occurs with two different levels of another factor.] 
...
In Chapter \@ref(measurement), we discussed a set of validation strategies by which researchers can construct a validity argument [@kane1992]. How can we do this with respect to a manipulation? We recommended assessing measure validity by its relationship to other measures of the construct. The same comparison is possible for a manipulation -- you can ask whether the effect estimate yielded by the experiment is a valid measurement of the causal construct of interest.^[One caveat is that this question incorporates the validity of the manipulation *and* the measure. You can't really have a good estimate of a causal effect if the measurement is invalid.]
