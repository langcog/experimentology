...
We'll try to go easy on notation but some amount will make things clearer. We will use $\theta$ ("theta") to denote the parameter we want to estimate (the "estimand") and $\widehat{\theta}$ its sample estimate.[^estimation-4] This is the probability that the lady is correct in her judgment.
...
[^estimation-5]: In other problems, like multi-variable regression, the model is more complicated, for example involving more than one regression coefficient -- that's what we'll get to in Chapter \@ref(models).
...
We've now covered estimating a single parameter using both frequentist and Bayesian methods. But recall that what we really wanted to do was to estimate the causal effect we were interested in, namely the milk-first vs. tea-first effect. We could refer to this as our **treatment effect**. In practice, this is going to be a pretty straightforward extension.

In the maximum likelihood framework, we need to ...
...
In the previous section, we discussed how to estimate the mean and standard deviation of our observed data. These summary statistics are helpful for ...
