...
In this case, because of the confounds, we say that the observed correlation between money and happiness is a **biased**\index{bias (estimation)} estimate of the causal effect of money on happiness. The amount of bias introduced by the confounds can vary in different scenarios---it may only be small or it may be so strong that we conclude that there's a causal relationship between two variables when there isn't one at all.
