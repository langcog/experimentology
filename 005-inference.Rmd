...
Even with an actual treatment effect of zero, across repeated sampling, we should see some variation in $\hat{\Delta}$, our *estimate* of the treatment effect. Sometimes we'll get a small positive effect, sometimes a small negative one. Occasionally just by chance we'll get a big effect. Fisher's innovation was to quantify the probability of observing various values of $\hat{\Delta}$, given the null model. Then, if the observed data were very low probability under the null model, we could declare that the null was rejected. [MM: If describing repeated sampling here, maybe more later definition of this concept up to here as well. Overall, I wonder if all the material on sampling distributions should come immediately after the first section?]

How unlikely must the observed data be, in order to reject the null? Fisher declared that it is "usual and convenient for experimenters to take 5 percent as a standard level of convenience," establishing the .05 cutoff that has become gospel throughout the sciences.^[Actually, right after establishing .05 as a cutoff, Fisher then writes that "in the statistical sense, we thereby admit that no isolated experiment, however significant in itself, can suffice for the experimental demonstration of any natural phenomenon... in order to assert that a natural phenomenon is experimentally demonstrable we need, not an isolated record, but a reliable method of procedure. In relation to the test of significance, we may say that a phenomenon is experimentally demonstrable when we know how to conduct an experiment which will rarely fail to give us a statistically significant result." In other words, Fisher was all for replication!]
...
When we conduct a $t$-test on our experimental results, we see that the difference between the two groups is statistically significant at $p<.05$: `r papaja::apa_print(tea_t)$statistic`. Here we've given a standard American Psychological Association report of a $t$-test. The first part of this report on the test gives the $t$ value, qualified by the **degrees of freedom** for the test. We won't focus much on the idea of degrees of freedom here, but for now it's enough to know that this number quanifies the amount of information given by the data, in this case 48 datapoints minus the two means (one for each of the samples).  

Let's compare $p$ values and default Bayes Factors for a couple of different conditions. In Table \@ref(tab:p-bf-comparison), you can see a set of simulated experiments with varying total numbers of participants and varying average treatment effects. Both $p$ and BF go up with more participants and larger effects. But in general BFs tend to be a bit more conservative than $p$-values, such that $p<.05$ can sometimes translate to a BF of less than 3 [@benjamin2018]. 

```{r p-bf-comparison}
expand_grid(N = c(12,24,48,96), 
       Delta = c(.5, 1, 1.5)) |>
  rowwise() |>
  mutate(data = map2(N, Delta, 
                     ~make_tea_data(n_total = N, delta = Delta))) |>
  unnest(cols = c(data)) |>
  group_by(N, Delta) |>
  summarise(`p value` = t.test(x = rating[condition == "milk first"],
         ...
         BF = extractBF(
           BayesFactor::ttestBF(x = rating[condition == "milk first"],
                               y = rating[condition == "tea first"], 
                               paired = FALSE))$bf) %>%
  kable(booktabs = TRUE, digits = 3, caption = "Comparison of p-value and BF for several different tea-tasting scenarios.")
```
...
>>>>>>> e85f0732b18b7c707a17aa381a835a010b2dd2a1
...
One way to "patch" NHST is to introduce a decision-theoretic view, shown in Figure \@ref(fig:inference-power-alpha).[^inference-16] On this view, there is a real $H_1$, albeit one that is not specified. Then the true state of the world could be that $H_0$ is true or $H_1$ is true. The $p<.05$ criterion is the threshold at which we are willing to reject the null, and so this constitutes our **false positive** rate $\alpha$. But we also need to define a **false negative** rate, which is conventionally called $\beta$. 

Setting these rates is a decision problem: If you are too conservative in your criteria for the intervention having an effect, however, then you risk a false negative, where you incorrectly conclude that it doesn't work. And if you're too liberal in your assessment of the evidence, then you risk a false positive.[^inference-17] In practice, however, mostly people leave $\alpha$ at .05 and try to control their false negative rate by increasing their sample size. 
...
<!-- FINAL COMMENT ON NEYMAN PEARSON  -->
...
<!-- MM: This is the section (up to "Confidence Intervals") that I'd suggest moving to after the opening section. --> 
...
```{r}
# Note from NC: this is clunky first attempt and I haven't checked its accuracy...
# Also, because we repeat this, we should write a simulation function
# Also, to simulate

# define number of iterations
n = 10000

# create empty vector of ps
sample_p = rep(NA, n)

# fill empty vector with means
for(i in 1 : n){
  sample_p[i] = sample(x = c(0, 1),
                       size = 2,
                       replace = T,
                       prob = c(.70, .30)) %>% #  assume unfair coin
    mean()
}

# transform to dataframe
sample_p <- data.frame(x = sample_p)
...
```{r}
# Note from NC: this is clunky and SLOW first attempt and I haven't checked its accuracy...
# Also, because we repeat this, we should write a simulation function

# create empty vector of ps at n = 2, n = 30, n = 60
sim.DF <- data.frame(n = numeric(), 
                     p.est = numeric())

for (n in c(2, 15, 30, 60)){
  for (i in 1 : 1000){ #  1000 iterations
    p.est = sample(x = c(0, 1),
                   size = n,
                   replace = T,
                   prob = c(.70, .30)) %>% 
      mean()
...
<!-- MM: Maybe we can use "SE/standard error" vs "\widehat{SE}/estimated standard error" throughout. I wouldn't want to lose the idea that there really is a true standard error out there. It depends on n, but not because it's an estimate. -->

### Confidence intervals

<!-- MM: Flagging some missing values "XXX" in here. -->
...
::: {.exercise}
True or false? If we conducted the tea experiment with 500 participants instead of 48, then all else equal, we would expect tea ratings to cluster more closely around the sample mean in the experiment with 500 participants.
::: 
