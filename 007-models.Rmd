...
::: {.box .code}
Fititng a mixed effects model is also surprisingly simple. The first step is to prepare your predictors. In this case, we center the `age` predictor. 
...
The only big difference here is that we add random effects terms to the formula, which are notated `(1 | X)` meaning "a random intercept for each value of `X`.
:::
