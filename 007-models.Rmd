...
<!-- [MM: Changed $\sigma_{\text{milk-first}}$ to $\sigma$ throughout for consistency with Ch 5.] -->
...
```
<!-- [MM: Plot is wonderful. Just a few minor points: (1) Probably best to keep plot notation consistent with text after we decide about thetas vs. betas. ] -->
...
Y_i &= \theta_{\text{milk-first}} + \beta_{\text{tea-first}} X_{i,group} + \beta_{\text{age}} X_{i,age}  + \epsilon_i \\
\end{align}
...
```
<!-- [MM: Flagging another change in notational convention in table above - NOT SURE HOW TO DEAL WITH THIS] -->
...
$$
logit(Y_{it}) = \beta_{cntl} + \beta_{age} * X{i,age} + \beta_{exp} * X_{i,group} + \beta_{age*exp} * X_{i,age} * X_{i,group} + \gamma_i + \delta_t + \epsilon_{it}
$$

<!-- [MM: Flagging notation inconsistency with earlier models in this chapter; FIXME FIXME] -->
...
<!-- [MM: Generally I think it makes sense to transform GLM coefficients to get, e.g., ORs rather than log-ORs, and then interpret accordingly.] MCF: I am on the fence about this - I'm not sure if ORs here are that easy to interpret. I guess I like the transformation to probabilities in expectation a bit better? -->
...
<!-- [MM: Shouldn't the mean for the control condition be 1/3 in expectation?] MCF: yeah, they had a dispreference for the middle stimulus in the control condition. -->
...
<!-- [MM: Very cool simulation. Have we taught them what RMSE is, though?] MCF: no - elided this in the caption. Good catch.  -->
