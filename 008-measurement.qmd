...
In the previous section of the book, we described a set of measurement-focused statistical techniques for quantifying (and maximizing) our precision. In this next set of three chapters focusing on planning experiments, we will develop our toolkit for designing the measures (this chapter), design manipulations (@sec-design), and sampling (@sec-sampling) strategies that will allow us to create and evaluate experiments. These chapters form a core part of our approach to "experimentology": a set of decisions to [reduce bias]{.smallcaps}\index{bias reduction}, maximize [measurement precision]{.smallcaps}\index{measurement precision}, and assess [generalizability]{.smallcaps}\index{generalizability}. Let's begin with measurement.
...
Many of the early debates over measurement revolved around the emerging area of *psychophysics*, the problem of relating objective, physical stimuli (e.g., light, sound, pressure) to the subjective sensations they produce in the mind.
...
![Computing test-retest reliability.](images/measurement/trt.png){#fig-measurement-trt .column-margin fig-alt="A diagram of two line segments with pairs of points from each connected."}
...
\noindent where $\sigma^2_w$ is the within-subject variance in measurements and $\sigma^2_b$ is the between-subject variance in the measurements. (The denominator of the ICC\index{intra-class correlation coefficient (ICC)} comes from partitioning the total observed variance $\sigma^2_o$ in the reliability formula above.)
...
Let's take a look at how we might make an argument about the validity of the CDI\index{Communicative Development Inventory (CDI)}, the vocabulary instrument from our case study. 

First, the CDI\index{Communicative Development Inventory (CDI)} is face valid---it is clearly about early language ability. In contrast, even though a child's height would likely be correlated with their early language ability, we should be skeptical of this measure due to its lack of face validity.\index{face validity} In addition, the CDI shows good convergent\index{convergent validity} and predictive validity.\index{predictive validity} Concurrently, the CDI correlates well with evidence from transcripts of children's actual speech and from standardized language assessments (as discussed in the case study above). And predictively, CDI scores at age two relate to reading scores during elementary school [@marchman2008].
...
Experimentalists sometimes have a tendency to make up ad hoc measures on the fly. It's fine to invent new measures, but the next step is to think about what evidence there is that it's valid! @Tbl-flake-questions gives a set of questions to guide thoughtful reporting of measurement practices [adapted from @flake2020].
...
@Fig-measurement-considerations shows a continuum between simple and complex behaviors. The simplest measurable behaviors tend to be button presses, for example:
...
Sometimes the easiest way to elicit information from participants is simply to ask. Surveys are an important part of experimental measurement, so we'll share a few best practices, primarily derived from @krosnick2010.\index{survey}
...
![Likert scales based on survey best practices: a bipolar opinion scale with seven points and a unipolar frequency scale with five points. Both have all points labeled.](images/measurement/likert.png){#fig-measurement-likert width="75%" fig-alt="A diagram of two line segments: 7 ticks from extremely bad to extremely good; 5 ticks from never to always."}
...
Recently some researchers have begun to use "visual analog scales" (or sliders) as a solution. We don't recommend these---the distribution of the resulting data is often anchored at the starting point or endpoints [@matejka2016], and a meta-analysis\index{meta-analysis} shows they're a lot lower than Likert scales\index{Likert scale} in reliability [@krosnick2010].
...
In sum, survey questions can be a helpful tool for eliciting graded judgments about explicit questions. The best way to execute them well is to try and make them as clear and easy to answer as possible.\index{survey}
:::
