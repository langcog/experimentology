...
In this chapter, we will explore how to use visualizations in our own carefully controlled behavioral experiments that license causal inferences. 
:::
...
```
ggplot(aes(x = condition, y = response, color = ..., linetype = ..., ))
```
...
Now that you've mapped your design clearly to the figure's axes, how do you decide which graphical elements to display?
In principle, you might think, these assignments are all arbitrary anyway: as long as we clearly label our choices, it doesn't matter whether we use lines, points, bars, colors, textures, or shapes. 
While it's true that there are many ways to show the same data, being thoughtful about our choices can make it much easier for readers to interpret our findings.
The second principle is to *facilitate comparison* along the dimensions relevant to our scientific questions. 
...
To illustrate, suppose we have a simple pre-post design with two groups: a treatment group and a control group.

```{r viz-prepost1}
library(ggthemes)
make_2x2_data <- function(n_total = 48, sigma = .5, delta = 1) {
  tibble(condition = c(rep("control", n_total/2),
                       rep("treatment", n_total/2)),
         phase = c(rep("pre", n_total/4), rep("post", n_total/4), rep("pre", n_total/4), rep("post", n_total/4)),
         rating = c(rnorm(n_total/4, mean = 2, sd = sigma),
                    rnorm(n_total/4, mean = 2, sd = sigma),
                    rnorm(n_total/4, mean = 2, sd = sigma),
                    rnorm(n_total/4, mean = 2 + delta, sd = sigma)))
} 
d <- make_2x2_data() %>%
...
d %>%
  ...
  ggplot(aes(x = phase, y = condition, fill = rating)) +
    geom_tile()
```
...
d %>%
  group_by(condition, phase) %>%
  summarize(rating = mean(rating)) %>%
  ggplot(aes(x = phase, y = condition, size = rating)) +
    geom_point()
```

These plots allow us to see that one condition is (qualitatively) bigger than others, but it's hard to tell how much bigger. 
Additionally, this way of plotting the data places equal emphasis on phase and condition, but we may instead have in mind particular contrasts, like the *change* from pre- to post-tests and how that change differs across conditions.
An alternative is to show four bars: two on the left showing the 'pre' phase and two on the right showing the 'post' phase.
Maybe the control group is red and the treatment group is blue.

```{r viz-prepost4}
d %>% 
  group_by(condition, phase) %>%
  summarize(rating = mean(rating)) %>%
  ggplot(aes(x = phase, y = rating, fill = condition)) +
...
However, we can do even better by noticing that the main question we are testing in our design is an *interaction*. 
...
We have to look between the red bars, and then look between the blue bars and implicitly judge one difference against the other: the actual statistic isn't explicitly shown anywhere! 
...
d %>% 
  group_by(condition, phase) %>%
  summarize(rating = mean(rating)) %>%
  ggplot(aes(x = phase, y = rating, color = condition, group = condition)) +
    geom_point() +
    geom_line()
```

Now the contrast we want to interpret is highlighted visually. 
...
The third principle of modern statistical visualization is therefore to *show the data* to visualize variability in some form.
...
```{r viz-show-data2}
d %>% 
  ggplot(aes(x = phase, y = rating, color = condition)) +
