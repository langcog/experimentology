...
In this section, we begin by introducing a few simple guidelines to keep in mind when making informative visualizations in the context of experimental psychology.^[For the purposes of understanding the examples in this chapter, it should be sufficient to work through our R tutorials for data manipulation and visualization in Appendices C and D.]
...
d %>% 
  group_by(condition, age_group) %>%
  summarise(rating = mean(rating)) %>%
  ggplot(aes(x = condition, y = rating, fill = age)) +
...
d %>% 
  ...
  ggplot(aes(x = phase, y = empirical_stat, color = condition, group = condition)) +
    ...
    geom_line() +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0) +
    geom_jitter(aes(x = phase, y = rating, color = condition), alpha = 0.25, width = 0.1, data = d) +
...
d %>%
  ...
  ggplot(aes(x = age_group, y = empirical_stat, color = condition, group = condition)) +
    ...
    geom_text_repel(aes(label = condition, colour = condition), 
                    data = . %>% filter(phase == "post"), 
                    min.segment.length = Inf, nudge_x = -.7, nudge_y = -.5) +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0) +
    geom_jitter(aes(x = phase, y = rating, color = condition, fill = condition), alpha = 0.25, width = 0.05, data = d) +
...
But, at least in our experience, this kind of visualization plays a much more ubiquitous role in a researcher's day-to-day activities.
...
The problem arises when we operationalize a theory's predictions in terms of a single statistic (e.g. a difference between groups or a regression coefficient) and lose track of everything else that may be going on.
Visualization forces us to zoom out and take in the bigger picture.
...
As an example, let's consider the data reported by @blake2015ontogeny in [Ontogeny of fairness in seven societies](http://www.nature.com/nature/journal/v528/n7581/full/nature15703.html).
...
The confirmatory "design plot" would focus on contrasting developmental trajectories for Advantageous vs. Disadvantageous inequality.
However, this is an incredibly rich dataset, including 866 pairs from different age groups and different testing sites across the world which used subtly different protocols.
How might we go about our diagnostics?

### Check data collection details

Let's start by getting a handle on some of the basic sample characteristics.
For example, how many participants were in each age bin?

```{r ggplot-blake3}
d %>%
  mutate(age = floor(actor.age.years)) %>%
  group_by(age, actor.id) %>%
  tally() %>%
  group_by(age) %>%
  tally() %>%
...
For example, we can see that age ranges differ somewhat across sites: the cut-off is 11 in India but 15 in Mexico.
We can also see that age groups are fairly imbalanced: in Canada, there are eighteen 11-year-olds but only five 6-year-olds.

None of these properties are problematic, but it gives us a degree of awareness that might shape our downstream analytic decisions.
...
We'll aggregate at the participant-level, and check the frequency of different rejection rates, overall.
...
We immediately notice that the vast majority (44\%) of participants never reject in the entire experiment.
Zero rejections is the modal behavior.
...
We then check how many trials each participant is completing:
...
We find that there's some variation here: 69% of participants completed 8 trials, 30% of participant completed 6 trials, and a small number of participants have odd numbers like 7 or 10 trials.
...
India and Peru only have 6 trials but it appears that additional trials were added at other sites.
In a design where the number of trials was careful controlled, seeing unexpected numbers here (like the 7 or 10 trial bins) are clues that something else may be going on in the data.
For example, there may have been a coding error, or a response may have been double-counted due to a technical problem.
This visualization may lead us to look up the participant IDs in these bins and manually inspect their data to see what might be going on.

You can also learn a lot from looking at *pairwise* relationships in datasets with multiple continuous predictors.
By making scatter plots of the different variables, you may notice that some are strongly correlated with one another, and might start to worry about collinearity problems in regression models trying to tease them apart.
...
Let's start by just looking at the data from the largest site.
...
ms %>%
  ...
  summarise(decision = mean(decision, na.rm=TRUE),
            n = n()) %>%
  ggplot(aes(x = age, y = decision, col = country)) +
...
It's definitely a figure we'd put in a paper, but it helps us get a sense of the predicted  pattern.
...
Now let's rebin into 3-year age groups so that individual point estimates are a bit more reliable, and add the other countries back in.
...
d %>%
  ...
  summarise(decision = mean(decision, na.rm=TRUE),
            n = n()) %>%
  ggplot(aes(x = age, y = decision, col = condition)) +
    geom_point(aes(size = n)) +
...
This is now looking much closer to a quick-and-dirty version of a "design plot" we might include in a paper.
The DV (rejection rate) is on the y-axis, the primary variable of interesting (age) is on the x-axis, and the other elements of the design (country and trial type) are mapped to color and facets, respectively.
...
As you iterate through different exploratory visualization, *stay vigilant*!
...
You might notice a datapoint with an impossible value, such as an proportion greater than 1 or a reaction time less than 0.
...
Or you might notice that an attribute is missing for some values, and trace it back to a bug reading in the data or merging data frames (maybe there was a missing comma in our `csv` file).
...
For a more classical treatment, see @tukey1977exploratory and [@tufte2001visual,@tufte1997visual].
