...
```{r design-kovacs-original, boxfigure=TRUE}
#| label: fig-design-kovacs-original
#| fig-cap: "Original data from Kovacs et al. (2010). Error bars show 95\\% confidence intervals."
#| column: margin
...
```{r design-kovacs-replication, boxfigure=TRUE}
#| label: fig-design-kovacs-replication
#| fig-cap: "Data from a series of replications of Kovacs et al. (2010), including versions on the web (Experiments 1a and 1b) and in lab (Experiment 1c), as well as several variations on the format of responding (Experiments 2 and 3; 2AFC = two alternative forced choice) and an experiment where a large wall kept the agent from seeing the ball at all (Experiment 4)."
...
```{r design-young-design}
#| label: fig-design-young-design
#| fig-cap: "The 2x2 crossed design used in Young et al. (2007)."
#| column: margin
...
These conditions would in turn allow us to estimate both two-way and three-way interactions, enumerated in @tbl-design-three-way.

```{r design-three-way}
#| label: tbl-design-three-way
#| tbl-cap: "Possible effects in a hypothetical 2x2x2 experimental design with affect, belief, and outcome as factors."
...
Three-way interactions are just the beginning, though. If you have three factors with two levels each, you can estimate 7 total effects of interest, as in @tbl-design-three-way. If you have four factors with two levels each, you get 15. Four factors with three levels each gets you a horrifying 80 different effects!^[The general formula for $N$ factors with $M$ levels each is $M^N-1$.] This way lies madness, at least from the perspective of estimating and interpreting individual effects in a reasonable sample. 
...
```{r design-between}
#| label: fig-design-between
#| fig-cap: "A between-participants design."
#| column: margin
...
```{r design-within, out.width = "75%"}
#| label: fig-design-within
#| fig-cap: "A within-participants design, counterbalanced for order."
...
```{r design-between-within}
#| label: fig-design-between-within
#| fig-cap: "Pros and cons of between- vs. within-participant designs. We recommend within-participant designs when possible."
...
```{r design-rm-between, out.width = "75%"}
#| label: fig-design-rm-between
#| fig-cap: "A between-participants, repeated-measures design."
...
```{r design-pre-post, out.width = "60%"}
#| label: fig-design-pre-post
#| fig-cap: "A between-participants, pre-post design."
...
```{r design-dose-schema}
#| label: fig-design-dose-schema
#| fig-cap: "Three schematic designs. (left) Control and treatment are two levels of a nominal variable. (middle) Control is compared to ordered levels of a treatment. (right) Treatment level is an interval or ratio variable such that points can be connected and a parametric curve can be extrapolated."
...
```{r design-money1}
#| label: fig-design-money1
#| fig-cap: "Confounding order and condition leads to "
#| column: margin
...
```{r design-money2}
#| label: fig-design-money2
#| fig-cap: "Confounding between order and condition can be removed by counterbalancing or randomization."
#| column: margin
...
```{r design-jaeggi, boxfigure=TRUE}
#| label: fig-design-jaeggi
#| fig-cap: "The primary outcome graph for Jaeggi et al. (2008)."
#| column: margin
...
```{r design-jaeggi-disagg, boxfigure=TRUE}
#| label: fig-design-jaeggi-disagg
#| fig-cap: "The four sub-experiments of Jaeggi et al. (2008), now disaggregated. Panels show 8- (A), 12- (B), 17- (C), and 19-session (D) studies. Note the different scales and measures. RAPM = Raven's Advanced Progressive Matrices; BOMAT = Bochumer Matrizentest. From @redick2013."
...
```{r design-rosenthal}
#| label: fig-design-rosenthal
#| fig-cap: "Magnitudes of expectancy effects. From @rosenthal1994."
