...
First, before we dive in, we want to acknowledge that a project-based course may look very different depending on student level (undergraduate vs. graduate/post-doc level) and availability of resources at your institution for a course like this, namely in terms of TA support and course funding (for data collection). For most of this guide, we will assume that you have a similar setup to ours (i.e., teaching at the graduate/post-doc level and have course funding and TAs to support the course), but we have also spent some time considering ways to adjust the course to fit different student levels and availability of resources. If this applies to you, please find the section at the end called "## Scenarios for different course layouts" where we lay out some ideas for how to adjust your course. 

### Sample syllabus for a 10-week and 16-week version of the course

Depending on whether your university operates on a quarterly basis (as Stanford does), or on a semester basis (as most other universities do), the pace of the course will differ. For Psych 251, since we are on the quarter system, we use the 10-week syllabus shown below. However, we have also adapted this to a 16-week course given that it better represents a majority of other institutions' academic calendars. ^[For the 16-week course, we have assumed that classes meet twice a week.)]

**10-week syllabus:**
```{r}
library(tidyverse)
library(kableExtra)
syllabus10Week <- read_csv("data/Syllabus10Week.csv")
options(knitr.kable.NA = '')
syllabus10Week %>% 
  dplyr::select(-Week) %>% 
  kbl() %>% 
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive")) %>%
  pack_rows("Week 1", 1, 3, label_row_css = "background-color: #666; color: #fff;") %>%
  pack_rows("Week 2", 4, 6, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 3", 7, 9, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 4", 10, 12, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 5", 13, 15, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 6", 16, 18, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 7", 19, 21, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 8", 22, 24, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 9", 25, 27, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 10", 28, 30, label_row_css = "background-color: #666; color: #fff;") %>% 
...
**16-week syllabus:**
```{r}
syllabus16Week <- read_csv("data/Syllabus16Week.csv")
options(knitr.kable.NA = '')
syllabus16Week %>% 
  dplyr::select(-Week) %>% 
  kbl() %>% 
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive")) %>%
  pack_rows("Week 1", 1, 2, label_row_css = "background-color: #666; color: #fff;") %>%
  pack_rows("Week 2", 3, 4, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 3", 5, 6, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 4", 7, 8, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 5", 9, 10, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 6", 11, 12, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 7", 13, 14, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 8", 15, 16, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 9", 17, 18, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 10", 19, 20, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Week 11", 21, 22, label_row_css = "background-color: #666; color: #fff;") %>%
  pack_rows("Week 12", 23, 24, label_row_css = "background-color: #666; color: #fff;") %>%
  pack_rows("Week 13", 25, 26, label_row_css = "background-color: #666; color: #fff;") %>%
  pack_rows("Week 14", 27, 28, label_row_css = "background-color: #666; color: #fff;") %>%
  pack_rows("Week 15", 29, 30, label_row_css = "background-color: #666; color: #fff;") %>%
  pack_rows("Week 16", 31, 32, label_row_css = "background-color: #666; color: #fff;") %>%
  column_spec(3, border_right = T) %>% 
  column_spec(4, border_right = T)
```
### Grading
