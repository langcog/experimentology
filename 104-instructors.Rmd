...
color.me <- which(syllabi$Category == "Content + Skills (10 weeks)" & syllabi$Color!="white")

syllabi |>
  mutate(Chapter = str_replace(Chapter, "Chapter ", ""), 
         Chapter = ifelse(!is.na(Chapter), Chapter, "")) |>
  filter(Category == "Content + Skills (10 weeks)") |>
  select(-c(Category, Color)) |>
  knitr::kable(Caption = "A sample 10-week syllabus with both skills and content materials") |> 
...
```{r}
syllabi <- read_csv("data/Syllabi.csv")
color.me <- which(syllabi$Category == "Content ONLY (10 weeks)" & syllabi$Color!="white")

syllabi |>
  mutate(Chapter = str_replace(Chapter, "Chapter ", ""), 
         Chapter = ifelse(!is.na(Chapter), Chapter, "")) |>
  filter(Category == "Content ONLY (10 weeks)") |>
  select(-c(Category, Color)) |>
  knitr::kable(Caption = "A sample 10-week schedule with both skills and content materials") |> 
  row_spec(color.me, bold = F, color = "black", background = "#ebe8fc")
```
...
```{r}
syllabi <- read_csv("data/Syllabi.csv")
color.me <- which(syllabi$Category == "Content + Skills (16 weeks)" & syllabi$Color!="white")

syllabi |>
  mutate(Chapter = str_replace(Chapter, "Chapter ", ""), 
         Chapter = ifelse(!is.na(Chapter), Chapter, "")) |>
  filter(Category == "Content + Skills (16 weeks)") |>
  select(-c(Category, Color)) |>
  knitr::kable(Caption = "A sample 10-week schedule with both skills and content materials") |> 
  row_spec(color.me, bold = F, color = "black", background = "#ebe8fc")
```
...
```{r}
syllabi <- read_csv("data/Syllabi.csv")
color.me <- which(syllabi$Category == "Content ONLY (16 weeks)" & syllabi$Color!="white")

syllabi |>
  mutate(Chapter = str_replace(Chapter, "Chapter ", ""), 
         Chapter = ifelse(!is.na(Chapter), Chapter, "")) |>
  filter(Category == "Content ONLY (16 weeks)") |>
  select(-c(Category, Color)) |>
  knitr::kable(Caption = "A sample 10-week schedule with both skills and content materials") |> 
  row_spec(color.me, bold = F, color = "black", background = "#ebe8fc")
```
