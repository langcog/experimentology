...
For example, $p$-values don't tell us the probability of the data under a specific alternative hypothesis that we might be interested in -- that's the posterior probability $p(H_1 | \text{data})$. When our tea-tasting $t$-test yielded `r papaja::apa_print(tea_t)$statistic`, that's *not* the probability of the null hypothesis being true! And it's definitely not the probability of milk-first tea being better. 
