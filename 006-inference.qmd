...
![One example of the distribution of treatment effects under the null model (with N=9 per group). The red regions indicate the part of the distribution in  which less than 5% of observations should fall.](images/inference/p-region.png){#fig-inference-null-model .margin-caption width=70%}
...
![Standard decision matrix for the Neyman-Pearson approach to statistical inference.](images/inference/power-alpha.png){#fig-inference-power-alpha .column-margin}
...
For example, $p$-values don't tell us the probability of the data under a specific alternative hypothesis that we might be interested in -- that's the posterior probability $p(H_1 | \text{data})$. When our tea-tasting $t$-test yielded `r papaja::apa_print(tea_t)$statistic`, that $p$ is *not* the probability of the null hypothesis being true! And it's definitely not the probability of milk-first tea being better. 
...
[^inference-19]: The simplest and most versatile one, the Bonferroni correction, just divides .05 (or technically, whatever your threshold is) by the number of comparisons you are making. Using that correction, if you do 20 null experiments, you would have a `r round(100*(1 - .95^20)/20)`% chance of a false positive.
...
![Confidence intervals on each of the two condition estimates, as well as on the difference between conditions.](images/inference/cis.png){#fig-inference-condition-cis .margin-caption width=60%}
