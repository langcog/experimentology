...
The practical upshot is that the central limit theorem\indexC{central limit theorem} directly helps us characterize the uncertainty of sample estimates\indexC{sample estimate}. For example, when the sample size is reasonably large (approximately $n>30$ in the case of sample means) the standard error (i.e., the standard deviation of the sampling distribution) of a sample mean is approximately $\widehat{SE} = \sigma/\sqrt{n}$. The sampling distribution\indexC{sampling distribution} becomes narrower as the sample size increases because we are dividing by the square root of the number of observations.
