...
Fixed-effect meta-analysis uses a *weighted-average*, wherein larger, more precise studies are given more weight in the calculation of the overall effect size. Specifically, fixed-effect meta-analysis weighs studies by the inverse of their variance (i.e., the inverse of their squared standard error). This makes sense because larger, more precise studies have smaller variances, and thus get more weight in the analysis. The fixed-effect pooled estimate is:
...
[^9]: The standard error of the fixed-effect $\widehat{\mu}$ is simply $\frac{1}{\sum_{i=1}^k w_i}$. This can be used to construct a confidence interval or $p$-value.


### Limitations of fixed-effects meta-analysis

One of the limitations of fixed-effect meta-analysis is that it assumes that the effect size is, well, *fixed* across studies. In other words, fixed-effect meta-analysis assumes that there is a single effect size that all studies are estimating. This is a bold assumption. For example, imagine that intergroup contact (a) decreases prejudice when the groups work together to obtain a resource, but (b) increases prejudice when the groups compete for a resource. If we combined one study that examined intergroup cooperation with a study that examined intergroup competition, we would likely to conclude that the effect of intergroup contact is zero. However, is it reasonable to assume that these studies are examining the same fixed effect? Is it perhaps more useful to think of the *multiple* effects of integroup contact (e.g., one effect for intergroup cooperation and one effect for intergroup competition?) 

In Paluck et al.'s meta-analysis, it turns out that all studies examined intergroup cooperation. However, these studies still differed in other ways that could lead to different effects. For example, some studies recruited adult participants while others recruited children. If we assume that intergroup contact works different for adults vs. children, then it is misleading to talk about a single intergroup contact effect. Instead, we would say that the effects of intergroup contact varies across studies: an idea called **heterogeneity**.
...
Like fixed-effect meta-analysis, the random-effects estimate of $\widehat{\mu}$ is still a weighted average of studies' estimates: $$\widehat{\mu} = \frac{ \sum_{i=1}^k w_i \widehat{\theta}_i}{\sum_{i=1}^k w_i}$$
...
To mitigate this problem, meta-analysts often exclude studies that may be affected by within-study bias (for example, by excluding nonrandomized studies when confounding is of concern, as done by @paluck2019contact). After data have been collected, meta-analysts often also qualitatively assess studies' risks of bias using established rating tools [@art]. Additionally, meta-analysts often conduct sensitivity analyses that examine how much meta-analysis results change when excluding certain types of studies [@art].

In Paluck et al.'s meta-analysis, one way they tried to counteract within-study biases was by only including studies that randomly assigned participants to receive a contact intervention. However, they could have gone a step further by coding for the extent to which, for example, participants may have differentially dropped out of the study. Doing so helps communicate just how much "garbage" is in the meta-analysis.
